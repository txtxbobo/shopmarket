<template>
  <div id="category">
    ...
    <slide-bar
      class="slide-bar"
      :data-list="slideBarData"
      @slideBarClick="slideBarClick($event)"
    ></slide-bar>
    <scroll class="category-scroll" ref="scroll" :probe-type="3">
      <category-top
        class="main-category"
        :main-top-data-list="topDataList"
      ></category-top>
      <tab-control
        :titles="['流行', '新款', '精选']"
        @tabClick="tabClick(currentIndex, $event)"
        ref="tab"
      ></tab-control>
...
export default {
  ...
  methods: {
    slideBarClick(obj) {
      ...
      this.getMainMoreData(
        this.slideBarData.list[this.currentIndex].miniWallkey,
        "pop"
      );
    },
    getSlideBarData() {
